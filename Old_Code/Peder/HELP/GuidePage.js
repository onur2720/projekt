/*  We import the same class from Tours.js
    We use the contructor function which includes the object 'this'. 'this' is the substitute for the new tour-values */
...
/*  We use getElementById to get the values written in the attribute in our HTML-document */
...
function CreateTour() {
    if (localStorage.getItem("tours") == null) {
        oldTours = []
        //First we use 'if', which checks that there is NO key named 'Users' in localStorage with the method getItem.
        // If this is true, the function will place an empty array to the variable allUsers
    } else {
        oldTours = JSON.parse(localStorage.getItem("tours"))
        /*If a key is named Users in localStorage, we use JSON.parse and the method getItem,
        in order to assign the value of Users to all Users. */
    }
